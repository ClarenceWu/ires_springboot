package tw.idv.ires.web.appointment.util;
...
public class AppointmentUtils {
    public boolean isPastAppointment(LocalDate appointmentDate, int timePeriod, Clinic clinic) {
        ...
        switch (timePeriod) {
            case 1:
                periodStr = clinic.getMorning();
                break;
            case 2:
                periodStr = clinic.getAfternoon();
                break;
            case 3:
                periodStr = clinic.getNight();
                break;
            default:
                throw new IllegalArgumentException("無效的時段編號：" + timePeriod);
        }
